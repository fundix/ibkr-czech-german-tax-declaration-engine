...
### Fáze B — data existují, chybí jen UI (nejlepší poměr hodnota/cena) ✅ HOTOVO

> Dokončeno na větvi `feat/gap-plan-phase-b` (4 commity, 1194 testů zeleně, +26 nových).
> **Odchylky a nálezy:**
> - **B2** — `symbol` filtr na Položkách sdílí i párování opcí na underlying (dřív ho měla jen cesta přes `disposal_summary`), takže `?symbol=WBD` vrátí akcii i všech 5 kontraktů. Selecty Sekce/Stav ztratily submit-on-change — se šesti filtry v jednom formuláři je explicitní tlačítko předvídatelnější.
> - **B4** — příznak „nad smlouvu" má toleranci půl procentního bodu, ne nulu. Per-item strop se zaokrouhluje na haléře, takže CVS vychází na 15,03 % a bez tolerance by nesl falešné varování; skutečné mezery jsou o řád širší. Přesná částka se u takových řádků pořád zobrazuje.
> - **Reálný nález z B4:** EVO (SE) sráží 30 % proti smluvním 15 % → **209,33 Kč k vrácení ve Švédsku**, RYAAY (IE) 25 % → 10,67 Kč. Celkem 220,07 Kč za rok 2025.
> - Váhy v % používají jako jmenovatel **dlouhou stranu**, ne čisté jmění — vypsaná pozice je závazek, ne podíl na majetku, jinak by součet přesáhl 100 %.

- [x] **B1 (S)** — **Stránka `/results/{run_id}/disposals`** nad hotovým `disposal_summary()`. Tenký route + Jinja tabulka s `class="sortable"` + toolbar → P/L ranking, klikací sort a live filtr zdarma (table-enhance.js). Přidat parametr řazení — dnešní `abs(gain_loss)` desc ([services.py:2789-2791](src/webapp/services.py:2789)) řadí velkou ztrátu nad střední zisk.
...
- [x] **B3 (S)** — **Rozpad podle třídy aktiva + měny + sloupec váhy v %.** Tři groupby nad `portfolio.json`; váhový sloupec automaticky řaditelný. Závisí na **A1** (jmenovatel). K měnovému koláči napsat, že neobsahuje hotovost (`CASH_BALANCE` nikdy nedostane FIFO knihu, [calculation_engine.py:328](src/engine/calculation_engine.py:328)) a že `eoy_currency` u pozic bez EOY řádku tiše defaultuje na „USD" ([services.py:1096](src/webapp/services.py:1096), [:1133](src/webapp/services.py:1133), [:1139](src/webapp/services.py:1139)). `PercentOfNAV` nepoužívat (neexportovaný, NAV zahrnuje hotovost).
...
### Fáze C — reálná práce, bez externího zdroje ✅ HOTOVO

> Dokončeno na větvi `feat/gap-plan-phase-c` (3 commity, 1230 testů zeleně, +36 nových).
> **Odchylky a nálezy:**
> - **C1** — hloubka ITM se do stupně závažnosti záměrně nepromítá. Vypsaný put 27 % v penězích se 133 dny do expirace pořád nese reálnou extrinsickou hodnotu, takže by „elevated" bylo falešné varování. Hloubku nese řaditelný sloupec Moneyness. Riziko se nastavuje **jen** na vypsaných nohách — dnešní odznak hořel i na long kontraktech.
> - **C1** — přidán MCP nástroj `get_options` (žádný z dvanácti opce nepokrýval).
> - **C2** — `parse_option_key` zvládá oba styly klíčů; ověřeno, že u všech 18 reálných kontraktů dá stejné hodnoty jako FIFO kniha. Cesta z výpisu pozic přiznává dvě omezení: výpis nese jednu čistou pozici na kontrakt (současný long i short není vidět) a dny se počítají od dneška, ne od konce roku.
> - **C2 nález** — `fetch_statement` byl vázaný jako default argument, takže se nedal patchnout a test se skutečně dovolal na IBKR. Nyní se resolvuje při volání.
> - **C3** — ISIN prefix se u ADR **nepoužívá**: všech 5 ADR v knize má US ISIN, ale žádný emitent není americký. Bez toho by 138 416 Kč čínské a izraelské expozice spadlo pod USA. BYDDY/DIDIY/NICE (ADR bez dividendy) zůstávají „neznámé" místo dohadu.
> - **C3** — krok (b) je z pohledu kódu hotový; zbývá **akce na tvé straně**: zapnout `IssuerCountryCode` ve Flex query pozic a přepočítat rok. Do té doby jede rozpad na událostech a ISIN.
> - Opce jsou z rozpadu podle země vynechané — zeměpis kontraktu je zeměpis podkladu.

- [x] **C1 (M)** — **Riziko přiřazení opcí (moneyness).** Dávkově dotáhnout kotace distinktních underlyingů (znovupoužít `_fetch_quotes` pool z PR #44, [services.py:1082-1114](src/webapp/services.py:1082) — pozor, OPTION-skip žije na dvou místech a moneyness ho nesmí rozbít), rozdělit long/short nohu v UI ([dashboard_valuation.html:111-112](src/webapp/templates/partials/dashboard_valuation.html:111) — služba obě nohy už vrací), spočíst vnitřní hodnotu a označit short ∧ ITM ∧ ≤N dní se stupni závažnosti. K párování underlying→kontrakt použít otestovanou heuristiku [:2688-2702](src/webapp/services.py:2688). Číst `portfolio.json` přímo (jako `options_overview`), ne přes `get_live_portfolio`. Závisí na **A1**. Měkkost: underlying nemá vlastní měnu, zastupuje `eoy_currency` (pro reálná data správně).
